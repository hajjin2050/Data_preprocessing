...
def remove_images_with_low_green_pixels(image_folder, mask_folder, threshold=0.72):
    ...
    for image_file in image_files:
       try:
            ...
            preprocessed_image = preprocess_image_with_exr(image_path, 'temp.tif')

            # Load the preprocessed image
            #preprocessed_image = cv2.imread('temp.tif', cv2.IMREAD_GRAYSCALE)

            # Calculate the ratio of green pixels
            zero_ratio = np.count_nonzero(preprocessed_image) / (preprocessed_image.shape[0] * preprocessed_image.shape[1])
            green_ratio = np.count_nonzero(np.logical_not(preprocessed_image)) / (preprocessed_image.shape[0] * preprocessed_image.shape[1])
            
            # If the ratio of green pixels is below the threshold, remove the image and the mask
            if green_ratio < threshold:
                os.remove(image_path)
                os.remove(mask_path)
                print(f"{image_file} 1픽셀 비율 : {green_ratio} / 0픽셀 비율 : {zero_ratio}")
       except:
           print(f"{image_file} 읽을 수  없음")

# Call the function
remove_images_with_low_green_pixels('/data/cabbage_3v/Dataset/train/image', '/data/cabbage_3v/Dataset/train/mask')
